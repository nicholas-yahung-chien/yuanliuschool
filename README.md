...
- `tools/generate_vercel_config.py`: regenerate `site/vercel.json` + `site/route_fallback.html` from route map
...
`site/vercel.json` and `site/route_fallback.html` are generated by `tools/generate_vercel_config.py`.

The config enables:

- `cleanUrls` (keeps original URL style)
- `trailingSlash: false`
- filesystem-first routing for normal static paths
- fallback route loader for pages mapped to `site/__pages/*.html` (used when original URL paths are too long for local Windows filesystem paths)
